...
This project was written and reviewed in an environment **without Windows,
Office, or Visual Studio installed**, so:

- I could not compile it or produce a real `.exe`/installer here - VSTO
  requires the actual Office Primary Interop Assemblies and the VSTO SDK,
  both Windows-only, both installed as part of Visual Studio's
  "Office/SharePoint development" workload.
- Every `.cs` file is complete, real implementation code (not a stub), and
  I double/triple-checked the Excel Interop calls against the documented
  API. But the **`.sln`/`.csproj` were written by hand to match the standard
  VSTO template** rather than generated by Visual Studio itself, so small
  environment-specific details (exact `$(VSToolsPath)` import location,
  reference versions) can need a tweak on first open.
- **You will need to build this on a Windows machine with Visual Studio +
  the "Office/SharePoint development" workload, and Excel installed**, to
  get a working DLL and a real Setup.exe. Steps below.

## Fastest path to a guaranteed-working project (recommended)

Rather than opening `ExcelValidationAddIn.sln` directly and possibly fighting
project-file quirks, it's more reliable to let Visual Studio generate a
correct project for your machine and drop these files into it:

1. In Visual Studio: **File ▸ New ▸ Project ▸ Excel VSTO Add-in** (C#).
   Name it `ExcelValidationAddIn`.
   (If you don't see this template, install the **Office/SharePoint
   development** workload via the Visual Studio Installer first.)
2. Delete the template's default `Ribbon1.cs`/`.Designer.cs`/`.resx` if it
   added one.
3. Copy these folders/files from this package into the new project folder,
   then **Show All Files** in Solution Explorer and **Include in Project**:
   `Models/`, `Services/`, `UI/`, `Ribbon/` (including `Ribbon.xml`).
4. Replace the generated `ThisAddIn.cs` with the one from this package
   (keep the auto-generated `ThisAddIn.Designer.cs` as VS made it).
5. Right-click `Ribbon/Ribbon.xml` ▸ Properties ▸ **Build Action = Embedded
   Resource**.
6. NuGet: install **Newtonsoft.Json** (`Install-Package Newtonsoft.Json`).
7. Build (Ctrl+Shift+B), then press F5 - VS will launch Excel with the
   add-in loaded for debugging.

## If you open `ExcelValidationAddIn.sln` directly instead

It's included because it's a genuine, complete reference of every file and
setting the project needs. If Visual Studio reports a problem loading it
(most likely the `$(VSToolsPath)` import path, which varies by VS version),
right-click the project ▸ **Reload Project**, or fall back to the steps
above - copying working source files into a fresh template project takes a
few minutes and sidesteps any version mismatch entirely.
